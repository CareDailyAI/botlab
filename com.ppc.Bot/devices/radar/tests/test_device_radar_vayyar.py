...
class TestRadarVayyarDevice(unittest.TestCase):
    ...
    def test_device_vayyar_init(self):
        ...
        mut = RadarVayyarDevice(botengine, location_object, device_id, device_type, device_desc)
    ...
    def test_device_vayyar_attributes(self):
        ...
        botengine.logging_service_names = ["vayyar"]
        ...
        assert mut.did_update_leds(botengine) == False
        assert mut.did_update_volume(botengine) == False

        assert mut.get_fall_status(botengine) == None
        assert mut.get_previous_fall_status(botengine) == None
        assert mut.get_leds(botengine) == None
        assert mut.get_volume(botengine) == None
        assert mut.get_telemetry_policy(botengine) == None

        assert mut.get_previous_fall_status(botengine) == None
        assert mut.is_detecting_stability_event(botengine) == False
        ...
        assert mut.did_update_fall_learning(botengine) == False
        assert mut.get_fall_learning(botengine) == None
        ...
        assert mut.need_start_learning(botengine) == True
        ...
        assert mut.did_boot(botengine) == False
        assert mut.did_press_button(botengine) == False
        assert mut.button_press_duration(botengine) == False
        assert mut.get_learning_mode_end(botengine) == 0
        assert mut.get_falling_mitigator(botengine) == False
        ...
        assert mut.get_enable_above_th_point_telemetry_enables(botengine) == False
        assert mut.get_test_mode(botengine) == False
        assert mut.get_offline_mode(botengine) == True
        assert mut.get_dry_contacts(botengine) == None
        assert mut.get_demo_mode(botengine) == False
        assert mut.get_door_events(botengine) == False
        assert mut.get_out_of_bed_enabled(botengine) == False
        assert mut.get_sensitive_mode(botengine) == False
        ...
        assert mut.get_mounting_type(botengine) == RadarVayyarDevice.SENSOR_MOUNTING_WALL
        ...
        mut.measurements = {
            ...
            RadarVayyarDevice.MEASUREMENT_NAME_LEARNING_MODE_END: [
                (botengine.get_timestamp() + (2 * utilities.ONE_WEEK_MS), botengine.get_timestamp()),
            ],
            ...
            RadarVayyarDevice.MEASUREMENT_NAME_DRY_CONTRACTS: [
                ({"primary": {"mode": 0, "policy": 0}, "secondary": {"mode": 0, "policy": 0}}, botengine.get_timestamp()),
            ],
        ...
        assert mut.did_update_leds(botengine) == True
        assert mut.did_update_volume(botengine) == True
        ...
        assert mut.did_update_fall_position(botengine) == False
        assert mut.get_fall_positions(botengine) == {}
        assert mut.did_update_fall_learning(botengine) == True
        ...
        assert mut.did_stop_detecting_occupancy(botengine) == False
        assert mut.get_learning_mode_end(botengine) == botengine.get_timestamp() + (2 * utilities.ONE_WEEK_MS)
        ...
        assert mut.get_dry_contacts(botengine) == {"primary": {"mode": 0, "policy": 0}, "secondary": {"mode": 0, "policy": 0}}
        ...
        assert mut.did_update_room_boundaries(botengine) == True
        assert mut.get_mounting_type(botengine) == RadarVayyarDevice.SENSOR_MOUNTING_CEILING
        assert mut.get_room_boundaries(botengine) == {
            "x_min_meters": -1.0,
            "x_max_meters": 1.0,
            "y_min_meters": -1.0,
        ...
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_LOC_X].append((0, botengine.get_timestamp()))
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_LOC_Y].append((0, botengine.get_timestamp()))
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_LOC_Z].append((0, botengine.get_timestamp()))
        ...
        # Test results with additional measurements
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_STATUS].append((RadarVayyarDevice.FALL_STATUS_FINISHED, botengine.get_timestamp() - utilities.ONE_MINUTE_MS))
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_LOC_X].append((1, botengine.get_timestamp() - utilities.ONE_MINUTE_MS))
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_LOC_Y].append((1, botengine.get_timestamp() - utilities.ONE_MINUTE_MS))
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_LOC_Z].append((1, botengine.get_timestamp() - utilities.ONE_MINUTE_MS))

        assert mut.get_previous_fall_status(botengine) == RadarVayyarDevice.FALL_STATUS_FINISHED
        ...
        assert mut.is_detecting_stability_event(botengine) == True
        assert mut.is_detecting_fall(botengine) == False
        assert mut.did_stop_detecting_fall(botengine) == False
        assert mut.did_cancel_confirmed_fall(botengine) == False
        assert mut.need_start_learning(botengine) == False

        # Test results with different measurements
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_STATUS][0] = (RadarVayyarDevice.FALL_STATUS_DETECTED, botengine.get_timestamp())
        ...
        # Test results with different measurements
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_STATUS][0] = (RadarVayyarDevice.FALL_STATUS_CALLING, botengine.get_timestamp())
        ...
        # Test results with different measurements
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_STATUS][0] = (RadarVayyarDevice.FALL_STATUS_FINISHED, botengine.get_timestamp())
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_STATUS][1] = (RadarVayyarDevice.FALL_STATUS_CALLING, botengine.get_timestamp() - utilities.ONE_MINUTE_MS)
        ...
        # Test results with different measurements
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_STATUS][0] = (RadarVayyarDevice.FALL_STATUS_CANCELLED, botengine.get_timestamp())
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_FALL_STATUS][1] = (RadarVayyarDevice.FALL_STATUS_CONFIRMED, botengine.get_timestamp() - utilities.ONE_MINUTE_MS)
        ...
        # Test results with different measurements
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_OCCUPANCY][0] = (0, botengine.get_timestamp())
        ...
        assert mut.did_boot(botengine) == True
        assert mut.did_press_button(botengine) == True
        ...
        def send_command(device_id, name, value):
            botengine.get_logger("TEST").debug("|send_command()       name={}       value={}".format(name, value))
            botengine.get_logger("TEST").debug("|send_command() param_name={} param_value={}".format(param_name, param_value))
        ...
        def send_commands(device_id, params):
            botengine.get_logger("TEST").debug("|send_command()     params={}".format(params))
            botengine.get_logger("TEST").debug("|send_command() all_params={}".format(all_params))
            assert all_params == params
        botengine.send_commands.side_effect = send_commands

        try:
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_FALLING_SENSITIVITY, 1)
            mut.set_fall_sensitivity(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_CONFIRMED_TO_ALERT_TIMEOUT_SEC, 15)
            ...
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_TELEMTRY_POLICY, 0)
            mut.set_telemetry_policy(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_LEARNING_MODE_END, botengine.get_timestamp() + (2 * utilities.ONE_WEEK_MS))
            ...
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_PRESENCE_REPORT_MIN_RATE_MILLS, 5500)
            mut.set_reporting_rate_ms(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_PRESENCE_PERIODIC_REPORT, 1)
            mut.set_reporting_enabled(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_SILENT_MODE, 1)
            mut.set_silent_mode(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_FALLING_MITIGATOR_ENABLED, 1)
            mut.set_falling_mitigator(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_ENTER_DURATION, 10)
            mut.set_enter_duration(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_EXIT_DURATION, 10)
            mut.set_exit_duration(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_DURATION_UNTIL_CONFIRM, 10)
            mut.set_duration_until_confirm_sec(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_MIN_TIME_OF_TAR_IN_FALL_LOC, 10)
            mut.set_min_time_of_target_in_fall_location(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_DRY_CONTRACT_ACTIVATION_DURATION, 10)
            mut.set_dry_contact_activation_duration_sec(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_ABOVE_TH_POINT_TELEMETRY, 1)
            ...
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_OFFLINE_MODE, 0)
            mut.set_offline_mode(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_CALLING_DURATION_SEC, 10)
            mut.set_calling_duration_sec(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_DRY_CONTRACTS, {"primary": {"mode": 0, "policy": 0}, "secondary": {"mode": 0, "policy": 0}})
            ...
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_DOOR_EVENTS, 1)
            ...
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_SENSITIVE_MODE, 1)
            mut.set_senstive_mode(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_SENSITIVITY_LEVEL, 0.5)
            mut.set_sensitivity_level(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_MIN_EVENTS_FOR_FIRST_DECISION, 1)
            mut.set_min_events_for_first_decision(botengine, param_value)
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_DETECTIONS_IN_CHAIN, 1)
            mut.set_num_of_detections_in_chain(botengine, param_value)
            all_params = [
                {'name': RadarVayyarDevice.MEASUREMENT_NAME_SENSOR_MOUNTING, 'value': RadarVayyarDevice.SENSOR_MOUNTING_CEILING},
                {'name': RadarVayyarDevice.MEASUREMENT_NAME_SENSOR_HEIGHT, 'value': 2.0},
            ]
            mut.set_ceiling_mount(botengine)
            all_params = [
                {'name': RadarVayyarDevice.MEASUREMENT_NAME_SENSOR_MOUNTING, 'value': RadarVayyarDevice.SENSOR_MOUNTING_WALL},
                {'name': RadarVayyarDevice.MEASUREMENT_NAME_SENSOR_HEIGHT, 'value': 1.5},
            ]
            ...
            mut.set_room_boundaries(botengine, {"x_min_meters": -1.0, "x_max_meters": 1.0, "y_min_meters": 1.0, "y_max_meters": 3.0, "z_min_meters": 0.5, "z_max_meters": 1.5, "near_exit": True})
            assert mut.near_exit == True
            all_params = [
                {'name': RadarVayyarDevice.MEASUREMENT_NAME_SENSOR_HEIGHT, 'value': 2.3},
                {'name': RadarVayyarDevice.MEASUREMENT_NAME_SENSOR_MOUNTING, 'value': RadarVayyarDevice.SENSOR_MOUNTING_CEILING},
                {"name": RadarVayyarDevice.MEASUREMENT_NAME_X_MIN, "value": -2.0},
                {"name": RadarVayyarDevice.MEASUREMENT_NAME_X_MAX, "value": 2.0},
                {"name": RadarVayyarDevice.MEASUREMENT_NAME_Y_MIN, "value": 0.3},
            ...
            mut.set_room_boundaries(botengine, {"mounting_type": RadarVayyarDevice.SENSOR_MOUNTING_CEILING})
            assert mut.near_exit == False
            all_params = [
                {'name': RadarVayyarDevice.MEASUREMENT_NAME_SENSOR_HEIGHT, 'value': 2.3},
                {'name': RadarVayyarDevice.MEASUREMENT_NAME_SENSOR_MOUNTING, 'value': RadarVayyarDevice.SENSOR_MOUNTING_CEILING},
            ...
            mut.set_room_boundaries(botengine, {"x_min_meters": -1.0, "x_max_meters": 1.0, "y_min_meters": 1.0, "y_max_meters": 3.0, "z_min_meters": 0.5, "z_max_meters": 1.5, "mounting_type": RadarVayyarDevice.SENSOR_MOUNTING_CEILING, "near_exit": True})
            assert mut.near_exit == True
    ...
    def test_device_vayyar_attributes_subregions(self):
        ...
        assert mut.get_subregion_index(botengine, {}) == None
        ...
        mut.measurements = {
            RadarVayyarDevice.MEASUREMENT_NAME_TRACKER_SUBREGIONS: [
                (json.dumps([{"xMin": -1.0, "xMax": 1.0, "yMin": -1.0, "yMax": 1.0, "zMin": 0.0, "zMax": 2.0, "isFallingDetection": True, "isPresenceDetection": True, "enterDuration": 120, "exitDuration": 120, "isLowSnr": True, "isDoor": False, "name": "test"}], separators=(',', ':')), botengine.get_timestamp()),
            ],
        ...
        assert mut.get_raw_subregions(botengine) == [{"xMin": -1.0, "xMax": 1.0, "yMin": -1.0, "yMax": 1.0, "zMin": 0.0, "zMax": 2.0, "isFallingDetection": True, "isPresenceDetection": True, "enterDuration": 120, "exitDuration": 120, "isLowSnr": True, "isDoor": False, "name": "test"}]
        subregion = {"x_min_meters": -1.0, "x_max_meters": 1.0, "y_min_meters": -1.0, "y_max_meters": 1.0, "z_min_meters": 0.0, "z_max_meters": 2.0, "detect_falls": True, "detect_presence": True, "enter_duration_s": 120, "exit_duration_s": 120, "low_sensor_energy": True, "is_door": False, "name": "test",}
        assert mut.get_sorted_subregions(botengine, {device_id: [subregion]}) == (
            [{"xMin": -1.0, "xMax": 1.0, "yMin": -1.0, "yMax": 1.0, "zMin": 0.0, "zMax": 2.0, "isFallingDetection": True, "isPresenceDetection": True, "enterDuration": 120, "exitDuration": 120, "isLowSnr": True, "isDoor": False, "name": "test"}], 
            [subregion]
        )
        ...
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_OCCUPANCY_MAP][0] = ('000000', botengine.get_timestamp())
        mut.measurements[RadarVayyarDevice.MEASUREMENT_NAME_OCCUPANCY_MAP][1] = ('100000', botengine.get_timestamp() - utilities.ONE_HOUR_MS)
        ...
        try:
            param_name, param_value = (RadarVayyarDevice.MEASUREMENT_NAME_TRACKER_SUBREGIONS, [{"xMin": -1.0, "xMax": 1.0, "yMin": -1.0, "yMax": 1.0, "zMin": 0.0, "zMax": 2.0, "isFallingDetection": True, "isPresenceDetection": True, "enterDuration": 120, "exitDuration": 120, "isLowSnr": True, "isDoor": False,}])
        except Exception as e:
            assert False, "Exception: {}".format(e)
    ...
    def test_device_vayyar_sorted_subregions(self):
        ...
        subregion_tiny = (
            {"x_min_meters": -0.2, "x_max_meters": 0.2, "y_min_meters": -0.2, "y_max_meters": 0.2, "z_min_meters": 0.0, "z_max_meters": 2.0, "detect_falls": True, "detect_presence": True, "enter_duration_s": 120, "exit_duration_s": 120, "low_sensor_energy": True, "is_door": False, "name": "tiny",},
            {"xMin": -0.2, "xMax": 0.2, "yMin": -0.2, "yMax": 0.2, "zMin": 0.0, "zMax": 2.0, "isFallingDetection": True, "isPresenceDetection": True, "enterDuration": 120, "exitDuration": 120, "isLowSnr": True, "isDoor": False, "name": "tiny",}
        )
        subregion_small = (
            {"x_min_meters": -0.5, "x_max_meters": 0.5, "y_min_meters": -0.5, "y_max_meters": 0.5, "z_min_meters": 0.0, "z_max_meters": 2.0, "detect_falls": True, "detect_presence": True, "enter_duration_s": 120, "exit_duration_s": 120, "low_sensor_energy": True, "is_door": False, "name": "small",},
            {"xMin": -0.5, "xMax": 0.5, "yMin": -0.5, "yMax": 0.5, "zMin": 0.0, "zMax": 2.0, "isFallingDetection": True, "isPresenceDetection": True, "enterDuration": 120, "exitDuration": 120, "isLowSnr": True, "isDoor": False, "name": "small",}
        )
        subregion_medium = (
            {"x_min_meters": -1.0, "x_max_meters": 1.0, "y_min_meters": -1.0, "y_max_meters": 1.0, "z_min_meters": 0.0, "z_max_meters": 2.0, "detect_falls": True, "detect_presence": True, "enter_duration_s": 120, "exit_duration_s": 120, "low_sensor_energy": True, "is_door": False, "name": "medium",},
            {"xMin": -1.0, "xMax": 1.0, "yMin": -1.0, "yMax": 1.0, "zMin": 0.0, "zMax": 2.0, "isFallingDetection": True, "isPresenceDetection": True, "enterDuration": 120, "exitDuration": 120, "isLowSnr": True, "isDoor": False, "name": "medium",}
        )
        subregion_large = (
            {"x_min_meters": -2.0, "x_max_meters": 2.0, "y_min_meters": -2.0, "y_max_meters": 2.0, "z_min_meters": 0.0, "z_max_meters": 2.0, "detect_falls": True, "detect_presence": True, "enter_duration_s": 120, "exit_duration_s": 120, "low_sensor_energy": True, "is_door": False, "name": "large",},
            {"xMin": -2.0, "xMax": 2.0, "yMin": -2.0, "yMax": 2.0, "zMin": 0.0, "zMax": 2.0, "isFallingDetection": True, "isPresenceDetection": True, "enterDuration": 120, "exitDuration": 120, "isLowSnr": True, "isDoor": False, "name": "large",}
        )
        subregion_bed = (
            {"x_min_meters": -1.0, "x_max_meters": 1.0, "y_min_meters": -1.0, "y_max_meters": 1.0, "z_min_meters": 0.0, "z_max_meters": 2.0, "detect_falls": True, "detect_presence": True, "enter_duration_s": 120, "exit_duration_s": 120, "low_sensor_energy": True, "is_door": False, "context_id": radar.SUBREGION_CONTEXT_BED, "name": "bed"},
            {"xMin": -1.0, "xMax": 1.0, "yMin": -1.0, "yMax": 1.0, "zMin": 0.0, "zMax": 2.0, "isFallingDetection": True, "isPresenceDetection": True, "enterDuration": 120, "exitDuration": 120, "isLowSnr": True, "isDoor": False, "name": "bed",}
        )
        subregion_door = (
            {"x_min_meters": -1.0, "x_max_meters": 1.0, "y_min_meters": -1.0, "y_max_meters": 1.0, "z_min_meters": 0.0, "z_max_meters": 2.0, "detect_falls": True, "detect_presence": True, "enter_duration_s": 120, "exit_duration_s": 120, "low_sensor_energy": False, "is_door": True, "name": "door",},
            {"xMin": -1.0, "xMax": 1.0, "yMin": -1.0, "yMax": 1.0, "zMin": 0.0, "zMax": 2.0, "isFallingDetection": True, "isPresenceDetection": True, "enterDuration": 120, "exitDuration": 120, "isLowSnr": False, "isDoor": True, "name": "door",}
        )
        assert mut.get_sorted_subregions(botengine, {device_id: [
            subregion_tiny[0],
            subregion_small[0],
            subregion_large[0],
            subregion_medium[0],
