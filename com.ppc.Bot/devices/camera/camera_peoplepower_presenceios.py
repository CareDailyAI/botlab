    ...
    def get_icon(self):
        ...
        if "ppc.robotConnected" in self.measurements:
            if len(self.measurements["ppc.robotConnected"]) > 0:
                if (
                    self.measurements["ppc.robotConnected"][0][0]
                    == self.ROBOT_PRESENCE_360
                ):
                    return "360"
                elif self.measurements["ppc.robotConnected"][0][0] == self.ROBOT_KUBI:
                    return "camera-kubi"
                elif (
                    self.measurements["ppc.robotConnected"][0][0] == self.ROBOT_GALILEO
                    or self.measurements["ppc.robotConnected"][0][0]
                    == self.ROBOT_GALILEO_BLE
                ):
