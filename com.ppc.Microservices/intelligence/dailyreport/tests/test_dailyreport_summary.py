...
class TestDailyReportSummaryMicroservice():
    ...
    def test_dailyreport_summary_daily_report_status_updated(self, is_active_mock):
        ...
        mut.daily_report_status_updated(botengine, {"report": {"period": DAILY_REPORT_ADDRESS, "sections": [{"title": "Wellness", "icon": "heart", "id": "wellness", "items": [{"id": "trend.care_score.0", "comment": "Care Score: 50%", "timestamp_ms": botengine.get_timestamp()}]},{"title": "Alerts", "icon": "alarm", "id": "alerts", "items": [{"id": "falls", "comment": "0 falls today", "timestamp_ms": botengine.get_timestamp()}]}]}, "status": dailyreport.REPORT_STATUS_COMPLETED, "metadata": {"some": "value"}})
        # TODO: Assert something...
    ...
    @patch('intelligence.dailyreport.location_dailyreport_summary_microservice.LocationDailyReportSummaryMicroservice._is_active')
    @patch('intelligence.dailyreport.location_dailyreport_summary_microservice.LocationDailyReportSummaryMicroservice._get_randomly_weighted_sections')
    @patch('signals.analytics.track')
    def test_dailyreport_summary_daily_report_sms_delivery(self, track_mock, get_randomly_weighted_sections_mock, is_active_mock):
        botengine = BotEnginePyTest({})
        # Clear out any previous tests
        botengine.reset()
        ...
        dailyreport.add_entry(botengine,location_object, dailyreport.SECTION_ID_ALERTS, comment=_("My Entry"), identifier="my_entry")
        ...
        assert any(item["comment"] == "My Entry" for item in items_to_notify if item.get("id", '') == "my_entry")

        # Add a new entry, to replace the previous days entry with the same identifier
        dailyreport.add_entry(botengine,location_object, dailyreport.SECTION_ID_ALERTS, comment=_("My New Entry"), identifier="my_entry")

        # Check the updated entry is provided during delivery
        def check_items(*args):
            botengine.get_logger("Test dailyreport").info("Side effect called: {}".format(args))
            assert args[0] == botengine
            assert args[1] == location_object
            assert args[2] is not None
            if args[2] == "dailyreport_summary_notify_professional_caregivers_delivered":
                assert args[3] is not None
                assert args[3].get("items_to_notify") is not None
                assert any(item["comment"] == "My New Entry" for item in args[3].get("items_to_notify") if item.get("id", '') == "my_entry")
            else:
                pass
        ...
        daylight_microservice = location_object.intelligence_modules["intelligence.daylight.location_daylight_microservice"]
        assert daylight_microservice.is_timer_running(botengine) == True
        botengine.fire_next_timer_or_alarm(daylight_microservice, reference=daylight_microservice.intelligence_id)
