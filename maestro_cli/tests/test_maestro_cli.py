...
class TestMaestroCLI():
    ...
    def test_maestro_cli_api(self):
        ...
        :return:
        """
        api.get_location = MagicMock(return_value={"addrStreet1": "123 Main", "addrCity": "Small Town", "zip": "12345", "id": 456, "name": "My Home", "timezone": {"id": "America/Los_Angeles", "offset": -480, "dst": True, "name": "Pacific Standard Time"}, "organizationId": 999})
        api.get_devices = MagicMock(return_value=[{"locationId": 456, "id": "abc123", "type": 123, "goalId": -1, "typeCategory": 789, "desc": "Device", "modelId": "devicemodel", "lastDataReceivedDate": "2022-12-12T04:10:03.492Z", "lastDataReceivedDateMs": 1670818203492, "lastMeasureDate": "2022-12-12T04:10:02.046Z", "lastMeasureDateMs": 1670818202046, "connected": False, "newDevice": False, "proxyId": "02000001300001FA", "startDate": "2021-05-23T14:57:19Z", "startDateMs": 1621781839000, "location": api.get_location()}])
        api.get_device_properties = MagicMock(return_value=[])
        api.get_system_property = MagicMock(return_value=None)
        api.get_current_device_parameters = MagicMock(return_value=[])

        tests_dir_path = os.path.join(os.getcwd(), 'tests')
        parameters_file = os.path.join(tests_dir_path, 'resources', 'abc123_parameters.csv')
        alerts_file = os.path.join(tests_dir_path, 'resources', 'abc123_alerts.csv')
        ...
        assert file_3 == os.path.join(tests_dir_path, 'resources', 'location_456_datastreams_history.csv')
        assert file_4 == os.path.join(tests_dir_path, 'resources', 'location_456_modes_history.csv')
        ...
        assert saved_files == [
            os.path.join(data_path, 'recording-location_456-7_days_of_data.json'), 
            os.path.join(data_path, 'recording__abc123_device_location-456.json')
        ]
