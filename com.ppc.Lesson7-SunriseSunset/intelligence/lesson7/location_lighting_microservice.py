...
class LocationLightingMicroservice(Intelligence):
    """
    ...
    def sunrise_fired(self, botengine, content=None):
        ...
        # I want to print out the SUNRISE notification once and send you a single push notification,
        # and tell you if you don't have any lights in your account one time. This location microservice will handle that.
        ...
        botengine.get_logger().info("{}SUNRISE for gateway ID {}!{}".format(Color.BOLD + Color.RED, content['proxy_id'], Color.END))
        botengine.notify(push_content="It's sunrise at gateway ID {}".format(content['proxy_id']))

        # Let's let you know you if you don't have any lights in your account.
        has_lights = False

        # Loop through all devices in this location to find and turn off all lights at sunrise.
        for device_id in self.parent.devices:

            # If this is a light device, and it is connected through the same physical gateway that the sunrise/sunset event is basing itself upon, then we have a light that can react.
            if isinstance(self.parent.devices[device_id], LightDevice) and self.parent.devices[device_id].proxy_id == content['proxy_id']:
                has_lights = True

        if not has_lights:
            botengine.get_logger().info("{}\tYou do not have any lights in your account to turn off.{}".format(Color.BOLD, Color.END))
    ...
    def sunset_fired(self, botengine, content=None):
        ...
        botengine.get_logger().info("{}SUNSET for gateway ID {}!{}".format(Color.BOLD + Color.RED, content['proxy_id'], Color.END))
        botengine.notify(push_content="It's sunset at gateway ID {}".format(content['proxy_id']))

        # Let's let you know you if you don't have any lights in your account.
        has_lights = False

        # Loop through all devices in this location to find and turn on all lights at sunset.
        for device_id in self.parent.devices:
        ...
        if not has_lights:
            botengine.get_logger().info("{}\tYou do not have any lights in your account to turn on.{}".format(Color.BOLD, Color.END))
