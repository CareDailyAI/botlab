...
#
# There are 2 methods available to make an bot execute again:
#
#   botengine.execute_again_in_n_seconds(<seconds>)
#   botengine.execute_again_at_timestamp(<timestamp in unix milliseconds>)
#
# In this bot, the Entry Sensor will execute the bot again in N seconds,
# and the Virtual Light Switch will execute the bot again at a fixed timestamp.
# 
#
...
def run(botengine):
    ...
    timestampMs = botengine.get_timestamp()          # Capture the timestamp of execution
...
# This is what our inputs look like when the bot executes again
#{
#  "access": [
#    {
#      "category": 4,
...
#}
    ...
    if triggerType == TRIGGER_TYPE_MEASUREMENT:
        ...
        botengine.save_variable("last_device_type", str(deviceType))
        botengine.save_variable("last_device_name", str(deviceName))
        
        # Load or create the appropriate object, with the device ID as part of the variable name to track multiple devices.
        focused_object = botengine.load_variable("sensor_object_" + str(deviceId))
        if focused_object == None:
            focused_object = Sensor()
            botengine.save_variable("sensor_object_" + str(deviceId), focused_object)
        ...
        if int(deviceType) == ENTRY_SENSOR_DEVICE_TYPE:
            ...
            if state:
                print("Your '" + deviceName + "' opened! Executing this bot again in 2 seconds...")
                botengine.execute_again_in_n_seconds(2)
            else:
        ...
        if int(deviceType) == VIRTUAL_LIGHT_SWITCH_DEVICE_TYPE:
            ...
            if state:
                print("Your '" + deviceName + "' switched on! Executing this bot at a specific timestamp 2 seconds from now...")
                botengine.execute_again_at_timestamp(int(timestampMs) + 2000)
            else:
    ...
    if triggerType == TRIGGER_TYPE_EXECUTE_AGAIN:
        ...
        deviceId = botengine.load_variable("last_device_id")
        deviceType = botengine.load_variable("last_device_type")
        deviceName = botengine.load_variable("last_device_name")
        focused_object = botengine.load_variable("sensor_object_" + str(deviceId))
