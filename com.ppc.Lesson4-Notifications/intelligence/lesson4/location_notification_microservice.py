...
class LocationNotificationMicroserivce(Intelligence):
    """
    This microservice will send an HTML-based email using a local custom template when your mode changes. It will include an inline image attachment.

    The template is loaded from `email_template.vm`.

    An image is loaded from `inline_image.png`, and then this image is transformed into base-64 encoding and included as an attachment in the email.
    The image is referenced by the template in a way that the image is rendered inside the email.
    """
    ...
    def mode_updated(self, botengine, current_mode):
        ...
        # This is the correct way to check for an explicit mode:
        #
        #    if "HOME" is in current_mode:
        #        # GOOD
        #        ...
        #
        # Do not do this:
        #
        #    if "HOME" == current_mode:
        #        # BAD
        #        ...
        #
        # Because the official mode string can include other attributes, like "AWAY.SILENT" and "HOME.DURESS".
        if "HOME" in current_mode:
            template = template.replace("$message", "Welcome Home.")
        elif "AWAY" in current_mode:
            template = template.replace("$message", "Goodbye.")
        elif "STAY" in current_mode:
            template = template.replace("$message", "Protecting your perimeter.")
        elif "SLEEP" in current_mode:
            template = template.replace("$message", "Goodnight.")
        elif "VACATION" in current_mode:
            template = template.replace("$message", "Have a nice trip.")
        else:
            template = template.replace("$message", "You are now in {} mode.")
        ...
        botengine.notify(
                        push_content = "Your bot says you are now in {} mode.".format(current_mode),
                        push_sound = None,
        ...
        botengine.get_logger().info("\t=> Push and email notifications sent on mode change")
